...
export default function MarketCard({ market, isTracked = false }: MarketCardProps) {
  ...
  return (
    <Link href={`/markets/${market.publicKey}`}>
      <div className={`
        rounded-2xl p-5 transition-all cursor-pointer
        ${dark
          ? 'bg-gradient-to-br from-white to-neon-purple/5 border-2 border-neon-purple shadow-[4px_4px_0px_0px_rgba(139,92,246,1)] hover:shadow-[2px_2px_0px_0px_rgba(139,92,246,1)]'
          : 'bg-white border-2 border-dark shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]'
        }
      ...
      `}>
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <span className={`
              ...
              ${dark ? 'border-neon-purple' : 'border-dark'}
              ${active ? (dark ? 'bg-neon-purple/20' : 'bg-neon-green') : 'bg-gray-200'}
            `}>
            ...
            {isTracked && !dark && (
              <span className="text-xs font-bold px-2 py-1 rounded-full bg-neon-green text-dark flex items-center gap-1">
        ...
        {/* Footer */}
        <div className={`flex items-center justify-between text-sm pt-3 border-t ${dark ? 'border-neon-purple/20' : 'border-dark/10'}`}>
          <div className={`flex items-center gap-1 ${dark ? 'text-neon-purple' : 'text-dark/60'}`}>
            <TrendingUp className="w-4 h-4" />
            <span>
              {(parseInt(market.account.initial_liquidity, 16) / 1_000_000).toFixed(2)} {dark ? 'DAC' : 'USDC'}
            </span>
          </div>
          <div className="flex items-center gap-1">
            {dark ? (
              <>
                ...
                <span className="text-neon-purple font-medium">Dark Market</span>
              </>
            ) : isTracked ? (
              <>
                <Zap className="w-4 h-4 text-neon-green" />
                <span className="text-neon-green font-medium">Tracked</span>
              </>
            ) : (
              <>
                <Users className="w-4 h-4 text-dark/60" />
                <span className="text-dark/60">Public Market</span>
              </>
            )}
          </div>
        </div>
